...
void GUIentity::RegisterGuiElement(GUI_BaseElement *_element, int _parentId)
{
	...
	Olog::assertThat([this,_element]() { return elements.find(_element->GetId()) == elements.end(); }, "Passed an already used UID for GUI element!");

	elements[_element->GetId()] = _element;
	bool childadded = AddChildToParent(_element, _parentId);
	Olog::assertThat([childadded]() { return childadded; }, "GUIentity: Failed to add child for unknown reason!");
}
...
GUI_BaseElement *GUIentity::GetElementById(int _id)
{
	...
	Olog::assertThat([this, i]() { return i != elements.end(); }, "GUIentity: Looking for nonexisting UID!");
	return i->second;
}
