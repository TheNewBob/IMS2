...
void IMS2::GetAddableComponents(vector<LOCATION_CONTEXT> &contexts, vector<IMS_Component_Model_Base*> &OUT_componentModels)
{
	// TODO Stub. One day this should check which components are actually available on the vessel, not just return them all.
	OUT_componentModels = IMS_ModuleDataManager::GetComponentModelsForContexts(contexts);
}
