...
{
	Olog::assertThat([plugin, this]() { return find(plugins.begin(), plugins.end(), plugin) != plugins.end(); }, "GUI_BaseElement: Attempting to remove plugin that does not exist!");
...
{
	Olog::assertThat([newstate]() { return newstate->sharers.size() == 0; }, "A state that is already shared cannot replace another state!");
	newstate->owner = this;
